...
| *`defaultUserPath`* __string__ | Path in Vault to access a KV secret with the fields username and password for the default user. For example "secret/data/rabbitmq/config".
| *`defaultUserUpdaterImage`* __string__ | sidecar container that updates the defaultUser's password in RabbitMq when it changes in Vault. Additionally, it updates /var/lib/rabbitmq/.rabbitmqadmin.conf (used by rabbitmqadmin CLI). A default image name and version is provided (rabbitmqoperator/admin-password-updater:0.1.1). Only override when the default image version is not appropriate. When set to empty string, it disables the sidecar and passwords are not updated in RabbitMQ. e.g. defaultUserUpdaterImage: ""`
