...
| *`secretBackend`* __xref:{anchor_prefix}-github-com-rabbitmq-cluster-operator-api-v1beta1-secretbackend[$$SecretBackend$$]__ | Secret auth backend configuration for the RabbitMQ cluster.
|===
...
SecretBackend configures a single secret backend from a list of available backends. Today only Vault exists as supported Secret backend. And k8s secret is the other implicit SecretBackend which will be used when we do not specify any Secret Backend Future backends could be SecretStoreCSIDriver
...
VaultSpec uses Vault annotations (see https://www.vaultproject.io/docs/platform/k8s/injector/annotations) which relies on an injected Vault Agent sidecar to retrieve secrets
...
| *`tlsSecretPath`* __string__ | Path to access a KV secret with the fields tlskey and tlscrt to enable TLS
| *`caSecretPath`* __string__ | Path to access a KV secret with the fields cacrt to enable mutual TLS
|===
