...
| *`defaultUserSecretPath`* __string__ | Path to access a KV secret with the fields username and password for the default user.
...
|===
| Field | Description
| *`pkiRolePath`* __string__ | Path to access PKI issuing path, e.g. "pki/issue/hashicorp-com" Required
| *`commonName`* __string__ | Specifies the requested CN for the certificate. Defaults to <serviceName>.<namespace>.svc if not provided
| *`altNames`* __string__ | Specifies the requested Subject Alternative Names, in a comma-delimited list. These can be host names or email addresses; they will be parsed into their respective fields.
