...
| *`defaultUserUpdaterImage`* __string__ | Sidecar container that updates the defaultUser's password in RabbitMQ when it changes in Vault. Additionally, it updates /var/lib/rabbitmq/.rabbitmqadmin.conf (used by rabbitmqadmin CLI). Set to empty string to disable the sidecar container.
